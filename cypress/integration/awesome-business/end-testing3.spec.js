...
describe('end testing', () => {
	...
	it("find iframe ", () => {
		cy.get(".sq-card-component").within(function ($iframe) {
			...
			cy.wrap(test).find("input[id='cardNumber']").type("4111 1111 1112 1311")
			cy.wait(2000)
			cy.wrap(test).find("input[id='cardNumber']").clear()
			cy.wait(2000)
			cy.wrap(test).find("input[id='cardNumber']").type("4111 1111 1111 1111")
			cy.wrap(test).find("input[id='expirationDate']").type("12/15")
			cy.wait(2000)
			cy.wrap(test).find("input[id='expirationDate']").clear()
			cy.wait(2000)
